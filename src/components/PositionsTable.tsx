...
interface Props { positions: Position[] | undefined; isLoading: boolean; }

export default function PositionsTable({ positions, isLoading }: Props) {
    ...
    <div className="card" style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div className="card-header">
        ...
        <span style={{
          ...
          color: rows.length > 0 ? 'var(--amber)' : 'var(--text-muted)',
        }}>
          {rows.length} open
        </span>
      </div>
    ...
    <tr className="animate-fade-in">
      <td>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ color: 'var(--text-primary)', fontWeight: 600 }}>{pos.pair}</span>
